...
pronameExpr pronameTbl =
  qual "proname || ';' || "
    ...
    <> oidArrayExpr (qual "proargtypes") "pg_catalog.pg_type" "oid" "typname"
    <> ", '{}'), ',')"
...
objRepQueryFor allRoles schemaSel schemaAlgoOpts schemaName tableName = \case
  HDatabaseSettings ->
    let nonAggCols =
          ["pg_encoding_to_char(encoding)", "datcollate", "datctype", "rolname"]
    in
      DbObjRepresentationQuery
        { objNameCol    = "datname"
        , repCols       =
        ...
        , identWhere    = Nothing
        , groupByCols   = ["datname", "_codd_privs.permissions"] ++ nonAggCols
        }
  ...
  HRole ->
    let nonAggCols =
          ...
          , ("db_privileges", "_codd_roles.permissions ->> pg_roles.rolname")
          ]
    in
      DbObjRepresentationQuery
  ...
  HView ->
    ...
    in
      hq
        { repCols = ("definition_md5", "MD5(pg_views.definition)") : repCols hq
        , joins   =
          joins hq
        ...
        -- TODO: Lots of columns still missing!! But pg_get_indexdef should do a good enough job for now
          ...
          repCols hq
            ++ [ ("unique"    , "indisunique")
               ...
               , ("definition", "pg_get_indexdef(pg_index.indexrelid)")
...
-- Joins for domain types
        ...
        , groupByCols   =
          map snd ckCols
            ++ ["pg_type.typname", "pg_type.typelem", "pg_type.typlen"]
        }
        -- === Do not include:
