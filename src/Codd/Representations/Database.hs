  ...
  fetch _ _ (objRepQueryFor, conn, allSchemas, allRoles, schemaAlgoOpts) =
    ...
    where
      ...
      combineQueriesWithWhere blockedFetches = do
        let allRepReqs :: [SameQueryFormatFetch]
            allRepReqs =
              zipWith
            ...
            queriesPerFormat ::
              [(QueryInPieces, NonEmpty SameQueryFormatFetch)]
            queriesPerFormat =
              flip map fetchesPerQueryFormat $ \sffs@(x :| _) ->
                let -- We use nested jsonb_insert because it throws errors for duplicate keys
                    jsonObject =
                      foldl'
                        ...
                        { selectExprs =
                          ...
                          nonIdentifyingWhere =
                            nonIdentWhere
                              (sqQueryObj x),
                          identifyingWheres =
                            case mapMaybe (identWhere . sqQueryObj) $
                              NE.toList sffs of
                              [] -> Nothing
                              fs -> Just $ interspBy True " OR " fs,
                          groupByExprs =
                            if null (groupByCols (sqQueryObj x))
                              then Nothing
                              else
                                Just $
                                  interspBy False ", " $
                                    "artificial_idx"
                                      : groupByCols (sqQueryObj x)
                        }
                 in (finalQip, sffs)
        ...
        forM_ queriesPerFormat $ \(qip, sffs) -> do
          ...
          let mergedResults =
                mergeResults
                  ( map (\sff -> (sqUniqueIdx sff, sqResults sff)) $
                      NE.toList sffs
                  )
                  allResults
...
readSchemaFromDatabase pgVer conn schemaSel allRoles schemaAlgoOpts = do
  ...
  where
    withEmptySearchPath f = do
      [currentSearchPath :: DB.Only Text] <-
        DB.query_
          conn
...
getNamespacesReps schemas =
  fmap Map.fromList $ for schemas $ \(schemaName, namespaceRep) -> do
    ...
    collations <-
      dataFetch $
        GetRepsReq HCollation (Just schemaName) Nothing
...
getTablesReps schemaName tables = for tables $ \(tblName, tableRep) -> do
  ...
  statistics <- dataFetch $ GetRepsReq HStatistics (Just schemaName) (Just tblName)
  pure $
    TableRep
      tblName
      tableRep
      ...
      (listToMap $ map (uncurry TableStatisticsRep) statistics)
