...
import Codd.Types (PgMajorVersion (..), TxnIsolationLvl (..))
...
queryServerMajorAndFullVersion :: (MonadIO m) => DB.Connection -> m (PgMajorVersion, Int)
queryServerMajorAndFullVersion conn = do
  -- strVersionMay <- liftIO $ PGInternal.withConnection conn $ \pqconn -> PQ.parameterStatus pqconn "server_version_num"
  -- let strVersion :: Text = decodeASCII $ fromMaybe (error "Could not find postgres server version") strVersionMay
  strVersion :: Text <-
  ...
  case Parsec.parseOnly (Parsec.decimal <* Parsec.endOfInput) strVersion of
    ...
    Right (numVersion :: Int) -> pure (PgMajorVersion (numVersion `div` 10000), numVersion)
