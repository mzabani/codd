...
data HashableObject = HSchema | HTable | HView | HRoutine | HColumn | HTableConstraint | HTrigger | HRole | HSequence | HPolicy
...
instance IsDbObject SchemaObjectHash where
    objName =
        \case
            TableHash n _ _ _ _ _ -> n
            ViewHash n _ -> n
            RoutineHash n _ -> n
            SequenceHash n _ -> n
    objHash =
        \case
            TableHash _ h _ _ _ _ -> h
            ViewHash _ h -> h
            RoutineHash _ h -> h
            SequenceHash _ h -> h
    hashFileRelativeToParent =
        \case
            TableHash n _ _ _ _ _ -> "tables" </> mkPathFrag n </> "objhash"
            ViewHash n _ -> "views" </> mkPathFrag n
            RoutineHash n _ -> "routines" </> mkPathFrag n
            SequenceHash n _ -> "sequences" </> mkPathFrag n
    childrenObjs =
        \case
            TableHash _ _ cols cks triggers pols -> map DbObject (Map.elems cols) ++ map DbObject (Map.elems cks) ++ map DbObject (Map.elems triggers) ++ map DbObject (Map.elems pols)
            ViewHash _ _ -> []
            RoutineHash _ _ -> []
            SequenceHash {} -> []
