...
-- This will use same transaction as the one used to apply the migrations to insert into codd_schema.sql_migrations.
            registerAppliedMigDefaultConnInTxnBlock
                ...
                -> txn ()
            registerAppliedMigDefaultConnInTxnBlock appliedMigrationName appliedMigrationTimestamp appliedMigrationDuration appliedMigrationNumStatements apStatus
                = do
...
-- and also will BEGIN..COMMIT-wrap the insertion using the default connection if it's available.
            registerAppliedMigIfPossibleOthers
                :: DB.Connection
                -> FilePath
                -> DB.UTCTimestamp
                -> DiffTime
                ...
                -> MigrationApplicationStatus
                -> m ()
            registerAppliedMigIfPossibleOthers blockConn appliedMigrationName appliedMigrationTimestamp appliedMigrationDuration appliedMigrationNumStatements apStatus
                = do
                    csExists <- readMVar coddSchemaExists
        ...
        singleInTxnBlockResult :: Maybe a <- foldM
            (\_ block -> do
                ...
                case
                    ...
                    of
                        (BlockInTxn inTxnBlock, True) ->
                            runInTxnBlockDefaultConn
                                (fmap Just . actionAfter hoistedBlocks)
                                conn
                                inTxnBlock
                                registerAppliedMigDefaultConnInTxnBlock
                        (BlockInTxn inTxnBlock, False) ->
                            if cinfo == defaultConnInfo
                                then runInTxnBlockDefaultConn
                                    (const $ pure Nothing)
                                    conn
                                    inTxnBlock
                                    registerAppliedMigDefaultConnInTxnBlock
                                else runInTxnBlockNotDefaultConn
                                    (const $ pure Nothing)
                                    conn
                                    inTxnBlock
  ...
  where
                      ...
                      withTransaction @txn txnIsolationLvl conn $ do
                          ...
                          errorOrOk <-
                              forMExcept hoistedMigs $ applySingleMigration
                                  conn
                                  (\fp ts duration numStmts apStatus ->
                                      lift $ registerMig fp
                                                         ts
                                                         duration
                                                         numStmts
    ...
    runInTxnBlockDefaultConn
        ...
        -> (  FilePath
           ...
           -> MigrationApplicationStatus
           -> txn ()
           ) -- ^ Using the `txn` is right: registering applied migrations happens in the default connection, and so it will happen in the same transaction as the migrations themselves.
        -> m b
...
registerRanMigration conn isolLvl fn migTimestamp appliedAt appliedMigrationDuration numAppliedStatements apStatus
    = let
          (args, timestampValue) = case (appliedAt, apStatus) of
              (NowInPostgresTime, NoTxnMigrationFailed) ->
                  ("?, clock_timestamp()", Nothing)
              (NowInPostgresTime, MigrationAppliedSuccessfully) ->
                  ("clock_timestamp(), ?", Nothing)
              (SpecificTime t, NoTxnMigrationFailed) -> ("NULL, ?", Just t)
              (SpecificTime t, MigrationAppliedSuccessfully) ->
                  ("?, NULL", Just t)
      in
