...
applySingleMigration conn afterMigRun isolLvl (AddedSqlMigration sqlMig migTimestamp)
    = do
        ...
        logInfo $ "BEFORE FIRST: " <> Text.pack (show initialTxnStatus)
                ...
                ((numStmts, mLastBegin, _) :> errorOrDone) <-
                    Streaming.fold
                        (\(!l, !lastBegin, !lastTxnStatus) txnStatusNow ->
                                            -- TODO: Properly compute lastBegin over fold, with it being Nothing
                                            -- if outside a BEGIN..COMMIT block and a number otherwise
                            traceShowId
                                $ (l + 1, , txnStatusNow)
                                $ case (lastTxnStatus, txnStatusNow) of
                                      (PQ.TransInTrans, PQ.TransInTrans) ->
                                          lastBegin
                                      (PQ.TransIdle, PQ.TransIdle) -> Nothing
                    ...
                    $ migrationSql sqlMig
                afterTxnStatus <- txnStatus conn
                logInfo $ "AFTER LAST: " <> Text.pack (show afterTxnStatus)
                case errorOrDone of
                        ...
                        noTxnMigRetryInstructions <- if migrationInTxn sqlMig
                            then pure Nothing
                                ...
                                logError
                                    ...
                                    <> " statements from no-txn migration "
                                    <> Text.pack fn
                                    <> ", the "
                                    <> Fmt.sformat Fmt.ords (numStmts + 1)
                                    <> " failed to be applied."
                                case mLastBegin of
                                    Nothing ->
                                        pure $ Just
                                            NoTxnMigMustRetryFailedStatement
                                    Just lastBeginNum -> do
                                        pure
                                            $ Just
                                            $ NoTxnMigMustRestartFromLastExplicitBegin
                                                  lastBeginNum
