    ...
    withConnection
      ...
      $ \conn ->
          void
            ...
            <> dbIdentifier (Text.pack $ connectDatabase migsConnString)
...
finallyDrop dbName f = f `finally` dropDb
 where
  dropDb = do
    ...
    withConnection
        connInfo { connectDatabase = "postgres", connectUser = "postgres" }
        testConnTimeout
      $ \conn ->
          void
            $  liftIO
...
-- A custom connection string is necessary because the DB doesn't yet exist
      , migrationCustomConnInfo = Just cinfo { connectUser     = "postgres"
