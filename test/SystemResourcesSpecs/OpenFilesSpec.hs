                  ...
                  filter
                    ( \l ->
                        ...
                        ( "migrations/open-files-limit"
                          ...
                          -- Our code opens files/folders that may not exist
                          -- and handles them not existing nicely.
                          -- We use this as an optimization to reduce one function call
                          -- that checks for existence in the hopes that this is one
                          -- fewer syscall
                          && not ("ENOENT" `Text.isInfixOf` l)
                    )
                    $ Text.lines contents
