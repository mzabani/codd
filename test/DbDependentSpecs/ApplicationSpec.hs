...
placeHoldersMig = AddedSqlMigration
  SqlMigration
    ...
    , migrationSql = mkValidSql "CREATE TABLE any_table();\n-- ? $1 $2 ? ? ?"
    , migrationInTxn          = True
...
createTableNewTableMig tableName inTxn migOrder = AddedSqlMigration
  SqlMigration
    { migrationName = "000" <> show migOrder <> "-create-table-newtable-mig.sql"
    , migrationSql = mkValidSql $ "CREATE TABLE " <> Text.pack tableName <> "()"
...
spec = do
  describe "DbDependentSpecs" $ do
    describe "Application tests" $ do
      describe "With the default database available" $ aroundFreshDatabase $ do
        ...
        it "Rows-returning function works for in-txn migrations"
          $ \emptyTestDbInfo -> do
              let (AddedSqlMigration mig t) = selectMig
                  inTxnMig = AddedSqlMigration mig { migrationInTxn = True } t
              void @IO $ runStdoutLoggingT $ applyMigrationsNoCheck
                emptyTestDbInfo
                (Just [inTxnMig])
                testConnTimeout
        ...
        it "COPY FROM STDIN works" $ \emptyTestDbInfo ->
          runStdoutLoggingT
              (applyMigrationsNoCheck
                ...
                testConnTimeout
                (\conn ->
                  liftIO $ DB.query conn "SELECT name FROM x ORDER BY name" ()
                )
              )
              ...
              it
                ...
                $ \emptyTestDbInfo -> do
                    let modifiedSettings =
                          emptyTestDbInfo { txnIsolationLvl = isolLvl }
                    ...
                    (actualTxnIsol :: DB.Only String, actualTxnReadOnly :: DB.Only
                        String) <-
                      runStdoutLoggingT @IO $ applyMigrationsNoCheck
                        modifiedSettings
                    ...
                    DB.fromOnly actualTxnReadOnly `shouldBe` "off"
                    DB.fromOnly actualTxnIsol `shouldBe` case isolLvl of
                      DbDefault       -> "read committed"
                      Serializable    -> "serializable"
                      RepeatableRead  -> "repeatable read"
                      ReadCommitted   -> "read committed"
                      ReadUncommitted -> "read uncommitted"

        it "Strict checking and lax checking behaviour on mismatched schemas"
          $ \emptyTestDbInfo -> do
              let bogusDbHashes = DbRep Aeson.Null Map.empty Map.empty
              void @IO
                ...
                $ \conn -> do
                    ...
                    runStdoutLoggingT
                        (applyMigrations
                          (emptyTestDbInfo { onDiskReps = Right bogusDbHashes })
                          (Just [createTableNewTableMig "newtable" True 1])
                          testConnTimeout
                          StrictCheck
                        )
                    ...
                    DB.query_
                        conn
                        "SELECT 1 FROM pg_catalog.pg_tables WHERE tablename='newtable'"
                      `shouldReturn` ([DB.Only 1] :: [DB.Only Int])
                      ...
                      void @IO
                        ...
                        $ \conn -> do
                            runStdoutLoggingT
                                (applyMigrations
                                  (emptyTestDbInfo
                                    { onDiskReps = Right bogusDbHashes
                                    }
                                  )
                                  (Just
                                    [ createTableNewTableMig t1 firstInTxn 1
                                    , createTableNewTableMig t2
                                                             (not firstInTxn)
                                                             2
                                    ]
                                  )
        ...
        it "no-txn migrations and in-txn migrations run in intertwined blocks"
          $ \emptyTestDbInfo -> do
              ...
              void @IO $ runStdoutLoggingT $ applyMigrationsNoCheck
                ...
                testConnTimeout
                (const $ pure ())
              withConnection (migsConnString emptyTestDbInfo) testConnTimeout
                $ \conn -> do
        ...
        context "Retry Policy retries and backoff work as expected" $ do
          let
            testRetries inTxn emptyTestDbInfo = do
                  ...
                  (applyMigrationsNoCheck
                    (emptyTestDbInfo
                      { retryPolicy   = RetryPolicy
                                          7
                                          (ExponentialBackoff (realToFrac 0.001))
                      , sqlMigrations =
                        [ if inTxn
                            then "test/migrations/retry-policy-test-in-txn/"
                            else "test/migrations/retry-policy-test-no-txn/"
                        ]
                      }
                    )
                    Nothing
              ...
              length (filter ("Retrying" `Text.isInfixOf`) logs) `shouldBe` 7
              ...
              forM_ [1, 2, 4, 8, 16, 32, 64] $ \delay ->
                length
                    (filter
                      (("Waiting " <> Text.pack (show delay) <> "ms") `Text.isInfixOf`
                      )
                      logs
                    )
                  `shouldBe` 1
      ...
      describe "Custom connection-string migrations" $ do
              ...
              finallyDrop "codd-test-db"
                ...
                $ do
                    runStdoutLoggingT $ applyMigrationsNoCheck
                      ...
                      testConnTimeout
                      (const $ pure ())
                    withConnection defaultConnInfo testConnTimeout $ \conn -> do
                      ...
                      runMigs :: [(FilePath, UTCTime)] <- DB.query
                        conn
                        "SELECT name, applied_at FROM codd_schema.sql_migrations ORDER BY applied_at, id"
                        ()
                      map fst runMigs
                        `shouldBe` map (migrationName . addedSqlMig) allMigs

                      -- Half a second is a conservative minimum given pg_sleep(1) in each migration
                      let
                        minTimeBetweenMigs = secondsToNominalDiffTime 0.5
                        migsWithSleep      = filter
                          (\(n, _) -> "-create-database-mig.sql"
                            `Text.isSuffixOf` Text.pack n
                          )
                          runMigs
                      zipWith
        ...
        it "Diverse order of different types of migrations" $ ioProperty $ do
          defaultConnInfo <- testConnInfo
          testSettings    <- testCoddSettings
          ...
          pure
            $ forAll
                (diversifyAppCheckMigs defaultConnInfo
                                       testSettings
                                       createCoddTestDbMigs
                )
            $ \(DiverseMigrationOrder allMigs) ->
                finallyDrop "codd-test-db"
                  ...
                  $ do
                      runStdoutLoggingT $ applyMigrationsNoCheck
                        testSettings
                        (Just $ map (hoistAddedSqlMigration lift) allMigs)
                        testConnTimeout
                        (const $ pure ())
                      withConnection defaultConnInfo testConnTimeout $ \conn ->
                        do
...
diversifyAppCheckMigs defaultConnInfo testSettings createCoddTestDbMigs = do
  ...
  createDbAndFirstDefaultConnMig <- sequenceA
