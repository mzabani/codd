...
spec = do
    ...
    describe "DbDependentSpecs" $ do
        describe "Hashing tests" $ do
            aroundFreshDatabase
                $ it
                      "Because our DB hashing is so complex, let's make sure operations we know should change schema compatibility do"
                $ \emptyDbInfo -> do
                      ...
                      foldM_
                          (\(hashSoFar, appliedMigs :: [AddedSqlMigration]) (nextMig, nextMigModifiesSchema) ->
                              do
                                  ...
                                  let
                                      migText = nonDestructiveSql
                                          $ addedSqlMig nextMig
                                  if nextMigModifiesSchema
                                      then do
                                          if hashSoFar == dbHashesAfterMig
                                              then do
                                                  print migText
                                                  hashDifferences
                                                          hashSoFar
                                                          dbHashesAfterMig
                                                      `shouldNotBe` Map.empty
                                                  hashSoFar
                                                      `shouldNotBe` dbHashesAfterMig
                                              else pure ()
                                      else do
                                          if hashSoFar /= dbHashesAfterMig
                                              then do
