              ...
              veryCloseUtcTimes =
                zip [0 ..]
                  $  map (UTCTime (fromGregorian 2020 1 1))
                         [0, 0.5, 1, 1.5, 2, 3, 4, 5]
                  ++ map (UTCTime (fromGregorian 2020 1 2))
                         [0, 0.03, 0.5, 0.84, 1, 1.5, 2, 3, 4, 5, 6, 7]
              migTimes = nubBy (\(_, t1, _) (_, t2, _) -> t1 == t2) $ map
                (\(n, t) ->
                  let (roundedUtcTime, dbTime) = toMigrationTimestamp t
                  in  (n, roundedUtcTime, dbTime)
                )
                veryCloseUtcTimes
            in
              do
                withConnection migsConnString testConnTimeout $ \conn -> do
                  void $ DB.executeMany
                    conn
                    "INSERT INTO timestamps (seq_number, tm1, tm2) VALUES (?, ?, ?)"
                    migTimes
                  timesFromDb1 :: [(Int, UTCTime, UTCTime)] <- DB.query
                    conn
                    "SELECT seq_number, tm1, tm2 FROM timestamps ORDER BY tm1"
                    ()
                  timesFromDb2 :: [(Int, UTCTime, DB.UTCTimestamp)] <- DB.query
                    conn
                    "SELECT seq_number, tm1, tm2 FROM timestamps ORDER BY tm1"
                    ()
                  timesFromDb2 `shouldBe` migTimes
                  map (\(n, t1, t2) -> (n, t1, DB.Finite t2)) timesFromDb1
