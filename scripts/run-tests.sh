...
if [[ $WITH_NIX ]]; then
    ...
    echo "Running tests on Postgres 15 with IOHK's haskell.nix derivation of codd"
    nix develop ".#testShells.x86_64-linux.pg15" $NIX_DEV_ARGS -c ./scripts/run-db-test.sh "$WITH_NIX"
fi
