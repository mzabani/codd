...
main = do
  ...
  where
    opts = info (cmdParser <**> helper) fullDesc
...
doWork dbInfo (Up mCheckSchemas connectTimeout) =
  runCoddLogger $ case mCheckSchemas of
    ...
    Just checkSchemas ->
      void $
        Codd.applyMigrations
          dbInfo
          Nothing
          connectTimeout
          checkSchemas
doWork dbInfo (Add destFolder verbosity fp) =
  runCoddLoggerLevelFilter verbosity $ void $ addMigration dbInfo destFolder fp
...
doWork _ PrintCoddVersion = putStrLn "0.2.0"
