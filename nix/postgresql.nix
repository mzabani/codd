# Copied from nixpkgs and modified to build statically by removing the "icu" dependency
let
  generic =
    # dependencies
    { stdenv, lib, fetchurl, makeWrapper, glibc, zlib, readline, openssl, lz4
    , systemd, libossp_uuid, pkg-config, libxml2, tzdata, libkrb5
    ...
    in stdenv.mkDerivation rec {
      ...
      buildInputs = [ zlib readline openssl libxml2 ]
      ...
      postInstall = ''
        ...
        # Commented out from nixpkgs's original because we do want static libs always available
