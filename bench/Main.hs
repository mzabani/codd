...
import Codd.Internal.MultiQueryStatement (forceStreamConcurrently)
import Codd.Parsing
  ( ParsedSql (..),
    PureStream (..),
    SqlMigration (..),
...
main = do
  ...
  hspec $ do
    it "Concurrent stream forcing speeds up migration application" $ do
      let parseStmt = S.Effect $ threadDelay 30_000 >> pure (S.Step $ 'a' :> S.Return ())
      let parsedStatementsStreamSimulation :: Stream (Of Char) IO () = parseStmt <> parseStmt <> parseStmt

      meas <-
        bench "Simulation of parsing statements in parallel to applying them" $
          S.toList_
            ( S.mapM
                ( \el -> do
                    -- Emulate applying a statement
                    threadDelay 50_000
                    pure el
                )
                $ forceStreamConcurrently 0 parsedStatementsStreamSimulation
            )
            `shouldReturn` ['a', 'a', 'a']
      -- The total time taken without forceStreamConcurrently would be the time to parse + the time to apply statements,
      -- which is 50_000*3+30_000*3.
      -- Since the second and third statements are parsed concurrently, though, and since the time to apply each
      -- statement is comfortably longer than the time to parse, the total time should be
      -- 30_000 + 50_000*3 = 180_000
      shouldBeF
        "Time to parse and apply concurrently"
        0.01 -- threadDelays have too much variance; this still bounds between 170ms and 190ms, which is precise enough
        (measTime meas / fromIntegral (measIters meas))
        0.18
    describe
