...
UPDATE employee SET
    very_experienced=experience='master'
    , very_immature=experience='junior'
...
$$
UPDATE employee SET experience2=CASE WHEN very_experienced THEN 'senior'::experience2 WHEN very_immature THEN 'junior' WHEN very_experienced IS NOT NULL THEN 'senior' END
    WHERE employee_id=(SELECT employee_id FROM employee WHERE (very_experienced IS NULL) <> (experience2 IS NULL) LIMIT 1);
$$
, 'employee', ARRAY['very_experienced', 'very_immature'], 'experience2', $$CASE WHEN NEW.very_experienced THEN 'senior' WHEN NEW.very_immature THEN 'junior' WHEN NEW.very_experienced IS NOT NULL THEN 'senior' END$$
);
